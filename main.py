...
# Хранилище последних отправленных рилсов
last_sent_reel = {}
...
def get_cancel_keyboard():
    """Кнопка отмены во время добавления"""
    keyboard = [
...
async def send_reel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not page:
        await query.edit_message_text(
            "❌ Нет доступных Reels со статусом 'Готов'.",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    # КРИТИЧНО: Меняем статус ПЕРЕД отправкой
    try:
        update_status(page_id, "Залит")
    except Exception as e:
        await query.edit_message_text(
            f"⚠️ Ошибка обновления статуса: {e}",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    if not hook.strip():
        await query.edit_message_text(
            f"⚠️ Запись {page_id[:8]} без хука, пропускаем.",
            reply_markup=get_after_reel_keyboard()
        )
        return
    ...
    # РАЗДЕЛИТЕЛЬ - видимая граница между рилсами
...
async def undo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /undo - для совместимости"""
    uid = update.effective_user.id
    if uid not in last_sent_reel or "page_id" not in last_sent_reel[uid]:
        await update.message.reply_text(
            "❌ Нечего откатывать.",
...
async def start_add_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало добавления через кнопку"""
    query = update.callback_query
    await query.answer()

    await query.edit_message_text(
        "📝 Введи хук и описание в одном сообщении.\n\n"
        "Первый абзац — хук, остальное — описание.\n"
        "Разделяй двойным переносом строки (Enter два раза).",
...
async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(hook) < 10:
        await update.message.reply_text(
            "⚠️ Хук слишком короткий (минимум 10 символов).\n"
            "Попробуй ещё раз:",
            reply_markup=get_cancel_keyboard()
        )
        return TEXT_INPUT
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            CommandHandler("add", start_add_command),
            CallbackQueryHandler(start_add_callback, pattern="^start_add$")
        ],
        states={
            TEXT_INPUT: [
        ...
        fallbacks=[
            CommandHandler("cancel", cancel_add_command),
            CallbackQueryHandler(cancel_add_callback, pattern="^cancel_add$")
        ]
    )
    app.add_handler(conv_handler)

    print("✅ Бот запущен. Пиши /start для главного меню.")
    app.run_polling()
